...
def test(*args, **kwargs):
    ...
    def _check_bcubed_score_corner_test_03():
        true_labels, predicted_labels, answer = np.array([1, 2]), np.array([1, 2]), 1.0
        return np.allclose(bcubed_score(true_labels, predicted_labels), answer, atol=1e-10, rtol=0.0)
    
    return _check_bcubed_score_corner_test_03(*args, **kwargs)
