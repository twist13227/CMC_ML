...
def check_test(data_dir):
    ...
    predictions = np.array(
        json.load(open(os.path.join(output_dir, "predictions.json")))
    )
    target = np.array(json.load(open(os.path.join(gt_dir, "target.json"))))

    if len(target) != len(predictions):
        res = json.dumps(
            {"status": "FAILED, inconsistent number of predictions", "mae": 20000.0}
        )
...
def grade(data_path):
    gradation = {5: [0, 2100], 3: [2100, 2200], 1: [2200, 2300], 0: [2300, 1e10]}
    student_mark = 0
    maes = []
    ...
    for i, result in enumerate(results):
        ...
        if status != "OK":
            mark_info = {
                "description": "An error occurred during predictions evaluation",
                "mark": 0,
            }
            if os.environ.get("CHECKER"):
...
def run_single_test(data_dir, output_dir):
    ...
    json.dump(
        list(predictions), open(os.path.join(output_dir, "predictions.json"), "w")
    )
...
if __name__ == "__main__":
    if os.environ.get("CHECKER"):
        ...
        shutil.copy(
            os.path.join(gt_dir, "target.json"),
            os.path.join(gt_dir, "gt", "target.json"),
        )
